...
class Roboclaw:
    def __init__(self, spine, devname, config, commands, sim):
        ...
        del self.spine.arduinos[devname]
        self.sim = sim
        

        if not sim:
            self.__roboclaw = RoboclawInner("/dev/{0:s}".format(self.devname), rate=115200)
...
class RoboclawInner:
    ...
    def __init__(self, comport, rate, timeout=1.0, retries=3):
